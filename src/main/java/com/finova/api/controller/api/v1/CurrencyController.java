...
import com.finova.api.service.CurrencyService;
...
@RestController
@RequestMapping("/api/v1/public/currencies")
...
//    }
    ...
    @GetMapping("all")
    @Operation(
            summary = "Get all currencies - no pagination",
            description = "Retrieve paginated list of all currencies. Requires authentication via Cookie (web) or Bearer token (mobile)."
    )
    public ResponseEntity<AppApiResponse<List<CurrencyResponse>>> getAllNoPagination() {
        List<CurrencyResponse> currencies = currencyService.getAllNoPagination();
        AppApiResponse<List<CurrencyResponse>> response = AppApiResponse.success(
                "Get all currencies successfully",
                currencies
        );
